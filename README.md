...
- 3年生：459読み・459書き（918問）
- 4年生：414読み・414書き（828問）
- 合計：873ペア、1,746問
- 素材版：`2026.09.08-1`／問題パック：`kanji-2026.09.08-1`
- `needs-fix` 素材54件は公開対象外

語句全体の読みは、表示文中の表面読みで管理します。たとえば「空き箱」は連濁するため、箱の回答は「はこ」ではなく「ばこ」です。問題IDと既存の学習履歴は維持します。
...
共通素材は `content-source/kanji-materials.json` と `content-review/*.json` で管理します。編集後に `npm run content:generate` を実行すると、`approved` 素材だけから `public/content/kanji-v2.json` とレビュー・カバレッジ表が生成されます。`needs-fix` 素材は理由を残したまま公開対象から除外します。
