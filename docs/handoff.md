...
- ブランチ：`main`
- 未コミットの変更：あり（「空き箱」の文中読み修正、生成物、レビュー記録、回帰テスト、ドキュメントを作業中。日本地図初版自体は`82036ee`でコミット・プッシュ済み）
- 最新素材版：`2026.09.08-1`
- 問題パック：`kanji-2026.09.08-1`
- 共通素材：927件
  - `approved`：873件（3年459・4年414）
  - `draft`：0件
  - `needs-fix`：54件（3年35・4年19）
- 公開問題：漢字は読み873問・書き873問の合計1746問、単位は453問、日本地図は47問
- 収録済みストロークデータ：532字（KanjiVG `r20250816` 由来、CC BY-SA 3.0。[ADR-0010](./adr/0010-kanjivg-stroke-data.md)）
- 自動テスト：203件成功（2026-09-08のローカル実測）
- 本番ビルド：成功（2026-09-08のローカル実測）。ビルド済みJavaScriptは2,660.10KB（gzip 1,018.47KB）で、500KB超過警告が続いている
- GitHub Pages：`https://minoru365.github.io/osarai-note/` で公開済み。2026-08-26に単位教科とモチベーション機能を反映し、公開サイトで「単位」カードが有効・ペット表示ありを確認した

3年生第2〜9バッチ、4年生第1・第2バッチ、承認済み873素材と生成物、確認画面生成スクリプト、固定470pxの書き回答領域は最新コミットまでに反映済み。GitHub Pagesの初回手動デプロイも成功している。作業開始時は必ず `git status --short` で利用者の変更を確認し、既存変更を破棄しない。
...
- ADR-0006/0012（教科横断ポイントとペット育成、公平な加算と装備進化）に基づき、DB v3データ層とホーム常駐UIを実装した（`docs/progress.md`参照）。
...
- 書き問題の例文整理（2026-09-07）：4年生の汎用的・重複した文脈を全件確認し、48件を語句に合う自然な文へ差し替え、25件を児童向けの代替語がない `needs-fix` へ移した。レビュー対象は208件で、汎用文脈と語句の文中重複は0件。
- `needs-fix` 代替語の反映（2026-09-08）：`群がる`、`折り紙`、`高低`、`海底`、`奈良`、`包丁`、`説き明かす`、`本望`、`学期末`、`末子`、`連ねる`、`冷気` の12件を `draft` に戻し、4年生のレビュー対象を220件、要修正を19件に整理した。
- 4年生一括レビューの反映（2026-09-08）：`kanji-g4-all` の220件をすべて承認し、素材版を `2026.08.15-6` へ更新した。必要なKanjiVG SVGを同じ `r20250816` から115字追加し、収録字は532字になった。生成後の公開問題は読み873問・書き873問。
- 日本地図初版（2026-09-08）：47都道府県の位置当て問題パックと、MITライセンスの都道府県境界SVGを追加した。ホームの「日本地図」から4年生の10問セットを開始でき、誤答・「分からない」・正解・当日進捗・1問1ポイントを既存の教科別保存モデルへ接続した。47問の人手レビューと実ブラウザ完走確認は次の作業。
- 「空き箱」の文中読み修正（2026-09-08）：表示文の連濁に合わせ、箱の出題読みを「はこ」から「ばこ」へ修正した。素材版 `2026.09.08-1` と公開問題を再生成し、問題IDは維持した。
- 日本地図クリック領域修正（2026-09-08）：素材SVGで非表示になっていた香川県と、`nigata`表記だった新潟県を読み込み時に正規化し、全47都道府県のクリック用テストを追加した。拡大・縮小・リセット、ピンチ操作、ドラッグ移動を実装し、ブラウザで香川県の選択、拡大、ドラッグ、リセットを確認した。
