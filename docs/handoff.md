...
- ブランチ：`main`
- 未コミットの変更：あり（モチベーション機能の3職種化、画像素材、UI、テスト、ADRの更新。ユーザー確認後にコミットする）
- 最新素材版：`2026.08.15-5`
...
- 自動テスト：192件成功（2026-09-06のローカル実測）
- 本番ビルド：成功（同上）。ビルド済みJavaScriptは2,146.97KB（gzip 819.83KB）で、500KB超過警告が続いている
