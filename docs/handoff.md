...
- ブランチ：`main`
- 未コミットの変更：あり（4年生220件の一括レビュー反映、KanjiVG SVG追加、生成ドキュメント、テストの更新。ユーザー確認後にコミットする）
- 最新素材版：`2026.08.15-6`
...
- 公開問題：漢字は読み873問・書き873問の合計1746問、単位は453問
- 収録済みストロークデータ：532字（KanjiVG `r20250816` 由来、CC BY-SA 3.0。[ADR-0010](./adr/0010-kanjivg-stroke-data.md)）
- 自動テスト：194件成功（2026-09-08のローカル実測）
- 本番ビルド：成功（2026-09-08のローカル実測）。ビルド済みJavaScriptは2,644.45KB（gzip 1,015.36KB）で、500KB超過警告が続いている
