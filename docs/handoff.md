...
- ブランチ：`main`
- 未コミットの変更：あり（日本地図初版の問題パック、都道府県SVG、クリック式10問画面、保存層、テスト、ドキュメント。ユーザー確認後にコミットする）
- 最新素材版：`2026.08.15-6`
...
- 自動テスト：200件成功（2026-09-08のローカル実測）
- 本番ビルド：成功（2026-09-08のローカル実測）。ビルド済みJavaScriptは2,657.24KB（gzip 1,017.47KB）で、500KB超過警告が続いている
...
2. ブラウザで地図のクリック判定、誤答後の再回答、「分からない」後の答え確認、10問完走、ホームの当日進捗を確認する
