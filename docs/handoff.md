...
- ブランチ：`main`
- 未コミットの変更：あり（日本地図クリック領域の修正とテストを作業中。日本地図初版自体は`82036ee`でコミット・プッシュ済み）
- 最新素材版：`2026.08.15-6`
- 問題パック：`kanji-2026.08.15-6`
- 共通素材：927件
...
- 自動テスト：202件成功（2026-09-08のローカル実測）
...
4年生の一括レビューは完了した。`kanji-g4-all` の220件をすべて承認し、素材版 `2026.08.15-6` と公開問題へ反映済みである。初期候補に残っていた「教科書に『〜』と書いてあります。」型218件も修正済みである。
